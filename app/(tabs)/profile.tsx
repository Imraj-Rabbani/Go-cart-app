    ...
    <SafeAreaView className='flex-1 bg-surface' edges={["top"]}>
      ...
      {!user ? (
        ...
        <View className='items-center mb-8'>
            ...
            <Text className='text-2xl font-bold text-gray-900 mb-2'>{user.firstName + " " + user.lastName}</Text>
        ...
        <View className='border border-gray-100 rounded-2xl p-2 bg-white shadow-sm '>
          {PROFILE_MENU.map((item) => (
            <TouchableOpacity key={item.id} className='flex-row items-center py-4 px-2 ' onPress={()=> router.push(item.route as any)}>
         ...
         {/* Logout */}
         <TouchableOpacity className='bg-red-500 py-3 px-6 rounded-full mt-4 flex-row justify-center items-center' onPress={handleLogout}>
