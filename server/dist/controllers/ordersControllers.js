...
const buildOrderFromCart = async (req, paymentMethod, clearCartAfterOrder) => {
...
export const initBkashPayment = async (req, res) => {
    try {
        ...
        const data = {
            ...
            product_name: order.items.map((item) => item.name).join(", ").slice(0, 200),
...
export const sslcommerzSuccess = async (req, res) => {
    try {
        ...
        if (order.paymentStatus !== "paid") {
            const validation = await getSSLCommerzClient().validate({ val_id: String(req.query.val_id || "") });
            ...
            order.paymentIntentId = String(req.query.val_id || order.paymentIntentId || "");
            await order.save();
...
const handleUnsuccessfulBkashPayment = async (req, res, status) => {
    try {
        const order = await Order.findById(req.params.orderId);
